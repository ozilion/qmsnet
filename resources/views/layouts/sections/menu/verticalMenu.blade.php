...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
  ...
  <ul class="menu-inner py-1">
    @foreach ($menuData[0]->menu as $menu)
      @php
        // Eğer role tanımlıysa ve "*" değilse, izin verilen rolleri diziye çeviriyoruz.
        $allowedRoles = [];
        if (isset($menu->role) && $menu->role !== '*') {
            $allowedRoles = explode('|', $menu->role);
        }
      @endphp
      ...
      @if (
            !isset($menu->role) ||
            $menu->role === '*' ||
            (isset($menu->role) && $menu->role !== '*' && in_array(Auth::user()->role, $allowedRoles))
          )
